...
void SQLiteClient::prepare_env() {
  ...
  db_path_ = db_name;
  sqllogfile << "[SQLite] Prepared environment with DB: " << db_path_
             << std::endl;
}
...
ExecutionStatus SQLiteClient::execute(
  ...
  if (auto conn = create_connection(); conn) {
    db = *conn;
    sqllogfile << "[SQLite] Connected to SQLite database" << std::endl;
  } else {
    sqllogfile << "Failed to connect to SQLite database" << std::endl;
    return kServerCrash;
  }

  // 각 쿼리를 처리
  for (const auto &q : queries) {
    sqllogfile << "[SQLite] Execute query: " << q << std::endl;

    sqlite3_stmt *stmt = nullptr;
    int result_code = sqlite3_prepare_v2(db, q.c_str(), -1, &stmt, nullptr);

    if (result_code != SQLITE_OK) {
      sqllogfile << "SQLite error preparing statement: " << sqlite3_errmsg(db)
                 << std::endl;
      close_connection(db);
      return kSyntaxError;
    }
  ...
  // 스키마 정보 가져오기
  sqllogfile << "[SQLite] Retrieving schema information..." << std::endl;
  ...
  if (schema_result != SQLITE_OK) {
    sqllogfile << "SQLite error retrieving schema: " << sqlite3_errmsg(db)
               << std::endl;
    close_connection(db);
    return kSyntaxError;
  }
  ...
  while (sqlite3_step(schema_stmt) == SQLITE_ROW) {
    ...
    sqllogfile << "Type: " << (type ? type : "NULL")
               << ", Name: " << (name ? name : "NULL")
...
void SQLiteClient::clean_up_env() {
  ...
  if (remove(db_name.c_str()) != 0) {
    sqllogfile << "Error deleting database file: " << db_name << std::endl;
  } else {
    sqllogfile << "[SQLite] Deleted database: " << db_name << std::endl;
  }
}
...
std::optional<sqlite3 *> SQLiteClient::create_connection() {
  sqlite3 *db = nullptr;
  sqllogfile << "[SQLite] Connecting to SQLite database: " << db_path_
             << std::endl;
  if (sqlite3_open(db_path_.c_str(), &db) != SQLITE_OK) {
    sqllogfile << "Failed to open SQLite database: " << sqlite3_errmsg(db)
               << std::endl;
    return std::nullopt;
  }
  return db;
